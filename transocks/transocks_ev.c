...
static void client_connect_timeout (int fd, short event, void *arg) {
  ...
  /* shut down client */
  client_remove (con, "timed out connecting to SOCKS5 server");
}
...
int main (int argc, char **argv) {
  ...
  while ((c = getopt (argc, argv, "vfp:H:s:S:c:h")) != EOF)
    switch (c) {
      ...
      case 'h': /* Print help */
        ...
        printf ("Usage: %s [-f] [-p Port] [-H IP-Address] [-s port] [-S hostname] [-c timeout] [-v]\n\n", argv [0]);
